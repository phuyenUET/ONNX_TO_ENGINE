...
def process_detections(output, input_shape, orig_shape, conf_thres=0.25, iou_thres=0.45):
    predictions = output[0].reshape(-1, 7)
    conf = predictions[:, 4]
...
def main():
    DET_ENGINE_PATH = "path/to/your/detection.engine"
    CLS_ENGINE_PATH = "path/to/your/classification.engine"
    INPUT_SOURCE = 0
    ...
    while True:
        ...
        boxes, scores, class_ids = process_detections(det_output, input_size, frame.shape[:2], conf_thres=0.5)
        for box, score, class_id in zip(boxes, scores, class_ids):
            ...
            if class_id == 1:
                ...
                if len(label_resized.shape) == 2:
                    label_resized = cv2.cvtColor(label_resized, cv2.COLOR_GRAY2BGR)
                label_input = np.transpose(label_resized, (2, 0, 1))
